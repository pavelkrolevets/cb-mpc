#include "ot_deterministic.h"

#include "ot.h"

namespace coinbase::mpc {

static const std::string kBaseOtRDet("base_ot_r_det");
static const std::string kBaseOtS0Det("base_ot_s0_det");
static const std::string kBaseOtT0Det("base_ot_t0_det");
static const std::string kBaseOtS1Det("base_ot_s1_det");
static const std::string kBaseOtT1Det("base_ot_t1_det");
static const std::string kExtRDet("ext_r_det");
static const std::string kPvwSDet("pvw_s_det");
static const std::string kSigma0Det("sigma0_det");
static const std::string kSigma1Det("sigma1_det");

// ----------------------- base_ot_protocol_pvw_ctx_det_t -----------------------------------

error_t base_ot_protocol_pvw_ctx_det_t::step1_R2S(const coinbase::bits_t& b) {
  cb_assert(!sid.empty());
  ...
  for (int i = 0; i < m; i++) {
    bn_t ri = crypto::ro::hash_number(sid, i, kBaseOtRDet).mod(q);
    if (ri == 0) ri = bn_t(1);
    r[i] = ri;
...
error_t base_ot_protocol_pvw_ctx_det_t::step2_S2R(const std::vector<buf_t>& x0, const std::vector<buf_t>& x1) {
  cb_assert(!sid.empty());
  ...
  for (int i = 0; i < m; i++) {
    if (rv = curve.check(A[i]))
      return coinbase::error(rv, "base_ot_protocol_pvw_ctx_det_t::step2_S2R: check A[i] failed");
    if (rv = curve.check(B[i]))
      return coinbase::error(rv, "base_ot_protocol_pvw_ctx_det_t::step2_S2R: check B[i] failed");

    bn_t s0 = crypto::ro::hash_number(sid, i, kBaseOtS0Det).mod(q);
    if (s0 == 0) s0 = bn_t(1);
    bn_t t0 = crypto::ro::hash_number(sid, i, kBaseOtT0Det).mod(q);
    if (t0 == 0) t0 = bn_t(1);
    U0[i] = curve.mul_add(s0, H0, t0);
    ecc_point_t X = ecc_point_t::weighted_sum(s0, A[i], t0, B[i]);
    V0[i] = crypto::ro::hash_string(X).bitlen(l) ^ x0[i];

    bn_t s1 = crypto::ro::hash_number(sid, i, kBaseOtS1Det).mod(q);
    if (s1 == 0) s1 = bn_t(1);
    bn_t t1 = crypto::ro::hash_number(sid, i, kBaseOtT1Det).mod(q);
    if (t1 == 0) t1 = bn_t(1);
    U1[i] = ecc_point_t::weighted_sum(s1, G1, t1, H1);
    X = ecc_point_t::weighted_sum(s1, A[i], t1, B[i]);
...
error_t base_ot_protocol_pvw_ctx_det_t::output_R(std::vector<buf_t>& x) {
  error_t rv = UNINITIALIZED_ERROR;
  ...
  for (int i = 0; i < m; i++) {
    if (rv = curve.check(U0[i]))
      return coinbase::error(rv, "base_ot_protocol_pvw_ctx_det_t::output_R: check U0[i] failed");
    if (rv = curve.check(U1[i]))
      return coinbase::error(rv, "base_ot_protocol_pvw_ctx_det_t::output_R: check U1[i] failed");
...
// ------------------------- transpose (duplicated from ot.cpp for static linkage) ---------------------

namespace transpose_det {
// NOLINTBEGIN
typedef union {
  uint8_t b[16];
  uint64_t l[2];
...
static void ot_matrix_transpose_det(const h_matrix_256rows_t& h_src, v_matrix_256cols_t& v_dst) {
  ...
  for (int i = 0; i < n_blocks; i++) {
    ...
    transpose_det::matrix_transpose(h_temp, byte_ptr(&v_dst[i * 256]), 256, 256);
  }
}

static buf_t hash_matrix_line_det(int index, buf256_t line, int l) {
  if (l == 256) return crypto::sha256_t::hash(index, line);
  return crypto::ro::hash_string(index, line).bitlen(l);
}

// ------------------------- ot_ext_protocol_ctx_det_t ---------------------

error_t ot_ext_protocol_ctx_det_t::step1_R2S(mem_t sid, const std::vector<buf_t>& sigma0,
                                             const std::vector<buf_t>& sigma1, const coinbase::bits_t& rr, int l) {
  ...
  buf_t ext_r_buf = crypto::ro::hash_string(sid, kExtRDet).bitlen(kappa + pad);
  r = rr + coinbase::bits_t::from_bin(mem_t(ext_r_buf));
  ...
  ot_matrix_transpose_det(T_tmp, T);
...
error_t ot_ext_protocol_ctx_det_t::step2_S2R(mem_t sid, const coinbase::bits_t& s, const std::vector<buf_t>& sigma,
...
error_t ot_ext_protocol_ctx_det_t::step2_S2R_sender_one_input_random(mem_t sid, const coinbase::bits_t& s,
                                                                     const std::vector<buf_t>& sigma,
                                                                     const std::vector<bn_t>& delta, const mod_t& q,
                                                                     std::vector<bn_t>& x0_out,
...
error_t ot_ext_protocol_ctx_det_t::step2_S2R_helper(mem_t sid, const coinbase::bits_t& s,
                                                    const std::vector<buf_t>& sigma,
                                                    const bool sender_one_input_random_mode,
                                                    const std::vector<buf_t>& x0, const std::vector<buf_t>& x1,
                                                    const std::vector<bn_t>& delta, const mod_t& q,
  ...
  for (int i = 0; i < u; i++) {
    ...
    uint8_t mask = -static_cast<uint8_t>(s[i]);
  ...
  ot_matrix_transpose_det(Q_tmp, Q);
  ...
  for (int i = 0; i < m; i++) {
    buf_t w0_bin = hash_matrix_line_det(i, Q[i], l);
    buf_t w1_bin = hash_matrix_line_det(i, Q[i] ^ s_buf, l);
...
error_t ot_ext_protocol_ctx_det_t::output_R(int m, std::vector<buf_t>& x) {
  ...
  x.resize(m);
  for (int i = 0; i < m; i++) {
    x[i] = hash_matrix_line_det(i, T[i], l);

    if (sender_one_input_random_mode) {
      if (bytes_to_bits(w1[i].size()) != l) return coinbase::error(E_BADARG, "sender_one_input_random_mode: w1 size");
      const buf_t w_sel = coinbase::ct_select_buf(r[i], w1[i], w_zero);
      x[i] ^= w_sel;
    } else {
      if (bytes_to_bits(w0[i].size()) != l || bytes_to_bits(w1[i].size()) != l)
        return coinbase::error(E_BADARG, "non-sender_one_input_random_mode: w0/w1 size");
...
// ------------------------------ ot_protocol_pvw_ctx_det_t ---------------------

error_t ot_protocol_pvw_ctx_det_t::step1_S2R() {
  coinbase::bits_t s = coinbase::bits_t::from_bin(mem_t(crypto::ro::hash_string(base.sid, kPvwSDet).bitlen(u)));
  return base.step1_R2S(s);
}

error_t ot_protocol_pvw_ctx_det_t::step2_R2S(const coinbase::bits_t& r, int l) {
  error_t rv = UNINITIALIZED_ERROR;

  std::vector<buf_t> sigma0(u);
  std::vector<buf_t> sigma1(u);
  for (int i = 0; i < u; i++) {
    sigma0[i] = crypto::ro::hash_string(base.sid, i, kSigma0Det).bitlen(ext.kappa);
    sigma1[i] = crypto::ro::hash_string(base.sid, i, kSigma1Det).bitlen(ext.kappa);
  }
...
error_t ot_protocol_pvw_ctx_det_t::step3_S2R(const std::vector<buf_t>& x0, const std::vector<buf_t>& x1) {
...
error_t ot_protocol_pvw_ctx_det_t::step3_S2R(const std::vector<bn_t>& x0, const std::vector<bn_t>& x1, int l) {
  cb_assert(x0.size() == x1.size());
...
error_t ot_protocol_pvw_ctx_det_t::step3_S2R(const std::vector<bn_t>& delta, const mod_t& q, std::vector<bn_t>& x0,
                                             std::vector<bn_t>& x1) {
  error_t rv = UNINITIALIZED_ERROR;
  std::vector<buf_t> sigma;
  if (rv = base.output_R(sigma)) return rv;
...
error_t ot_protocol_pvw_ctx_det_t::output_R(int m, std::vector<bn_t>& x) {
...
error_t ot_protocol_pvw_ctx_det_t::output_R(int m, std::vector<buf_t>& x) { return ext.output_R(m, x); }
